    ...
    def _get_elements(self) -> List:
        ...
        def extract_table(page):
            ...
            for idx, table in enumerate(tables):
                if table and table.cells:  # 确保表格非空
                    table_text = "\n".join(["\t".join(row) for row in table.cells if row])
                    table_texts.append(f"[表格 {idx + 1}]\n{table_text}")
        ...
        def pdf2text(filepath):
            ...
            for i, page in enumerate(doc):
                ...
                # table_content = extract_table(page)
                # if table_content:
                #     # print(table_content)
                #     page_resp += "\n".join(table_content)
                ...
                # print(resp)
                # print(resp.strip())
                if len(page_resp.strip()) < 10:
                    pix = page.get_pixmap(dpi=300)  # 整页转为图像
                    ...
                    result, _ = ocr(img_array)
                    if result:
                        ocr_text = [line[1] for line in result]
                        # page_content += ocr_text
                        page_resp += "\n".join(str(ocr_text))
                    else:
                        print(f"页面 {i + 1} OCR 未提取到内容")
...
class RapidOCRDocLoader(UnstructuredFileLoader):
    def _get_elements(self) -> List:
        ...
        # from unstructured.partition.text import partition_text
        #
        # print(self.unstructured_kwargs)
        #
        # return partition_text(text=text, **self.unstructured_kwargs)
