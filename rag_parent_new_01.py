...
本程序是一个基于 RAG（Retrieval-Augmented Generation）框架的知识库构建与问答系统，结合了文档加载、语义分块、向量化存储、
检索增强生成（RAG）等功能，能够实现基于文档内容的智能问答。程序支持多领域知识库的构建与切换，适用于智能问答、信息检索等场景。
...
1. 用户可选择是否重新构建知识库。
2. 用户可选择不同领域的知识库进行问答。
3. 用户输入问题后，系统基于知识库内容生成回答，并提供参考文档。
...
# ========== 以下内容将被迁移到rag_core.py ========== #
# 包括：excel_csv_to_chunks, load_document, load_documents_from_directory, llm_whole_doc_chunking, generate_child_chunks, build_knowledge_base, CustomRetriever, post_process_answer, clean_references, load_rag_pipeline, test_rag
...
def excel_csv_to_chunks(file_path, ext, chunk_size=20):
    """
    智能分块：每chunk_size行一个分块，保留表头，并自动生成摘要
    """
    from langchain.schema import Document
    try:
        if ext == ".csv":
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                for i in range(0, total_rows, chunk_size):
                    ...
                    summary = f"【Sheet】：{sheet_name}；【表头字段】：{', '.join(header)}；【本分块数据行数】：{len(chunk_df)}；【原始文件】：{os.path.basename(file_path)}"
                    content = summary + "\n" + text
                    chunks.append(Document(page_content=content, metadata={"source": file_path, "sheet": sheet_name, "chunk_index": i//chunk_size+1}))
            return chunks
        # CSV单表
        header = list(df.columns)
        total_rows = len(df)
        ...
        for i in range(0, total_rows, chunk_size):
            chunk_df = df.iloc[i:i+chunk_size]
            text = chunk_df.to_string(index=False)
            summary = f"【表头字段】：{', '.join(header)}；【本分块数据行数】：{len(chunk_df)}；【原始文件】：{os.path.basename(file_path)}"
            content = summary + "\n" + text
            chunks.append(Document(page_content=content, metadata={"source": file_path, "chunk_index": i//chunk_size+1}))
        return chunks
...
def load_document(file_path):
    ...
    elif ext in [".csv", ".xls", ".xlsx"]:  # 智能表格分块和摘要
        return excel_csv_to_chunks(file_path, ext, chunk_size=20)  # 可调整chunk_size
    else:
...
"""
def llm_whole_doc_chunking(documents, llm, max_input_length=500, overlap=300, Who_Are_You="全面专家"):
    # 定义分段 Prompt
    chunk_prompt = PromptTemplate(
...
def load_rag_pipeline(vector_store_path, deepseek_api_key, parent_map_path, WhoAreYou="全面专家"):
    ...
    # 加载嵌入模型和向量数据库
    # bge-m3
    embedding_model = HuggingFaceEmbeddings(model_name="../models/BAAI/bge-m3", model_kwargs={"device": "cuda:0"})
    # embedding_model = HuggingFaceEmbeddings(model_name="./BAAI/bge-m3", model_kwargs={"device": "cpu"})
    
    # bge-reranker-v2-m3
    # embedding_model = HuggingFaceEmbeddings(model_name="./BAAI/bge-reranker-v2-m3", model_kwargs={"device": "cuda"})
    # embedding_model = HuggingFaceEmbeddings(model_name="./BAAI/bge-reranker-v2-m3", model_kwargs={"device": "cpu"})
...
# 5. 测试查询（保持不变）
def test_rag(rag_chain, query, history=None):
    """
    history: [(user, ai), ...]
    """
    print(f"问题: {query}")
    ...
    if history:
        ...
        for i, (q, a) in enumerate(history):
            context_str += f"[历史第{i+1}轮] 用户: {q}\n[历史第{i+1}轮] AI: {a}\n"
        context_str += f"[当前] 用户: {query}"
        input_data = {"query": context_str}
    else:
        input_data = {"query": query}
    # 支持流式输出
    result = rag_chain.stream(input_data)
    answer = ""
    for re in result:
        # 假设re['result']为答案
        if isinstance(re, dict) and 'result' in re:
            answer += re['result']
    ...
    # 美化输出
    print("\n================= AI 回答 =================\n")
    body, refs = answer, None
    if "- 参考文档:" in answer:
        body, refs = answer.split("- 参考文档:", 1)
    print(body.strip())
    if refs:
        print("\n参考文档:")
...
if __name__ == "__main__":
    ...
    # 添加交互式选择
    while True:
        print("\n是否新建知识库？(yes/no) [默认 no]  (输入help查看说明, q退出)")
        choice = input(": ").strip().lower()
        if choice in ('q', 'quit', 'exit'):
            print("退出系统...")
            exit(0)
        if choice == 'help':
            print("输入yes将重新构建知识库（较慢），no则使用已有知识库。")
            continue
        if choice == '':
            choice = 'no'
        if choice == 'yes':
            ...
            print("请输入yes或no (或help)")

    while True:
        print("\n请选择知识库 (输入help查看说明, q退出):")
    ...
    while True:
        print("\n========================================================\n")
        print(f"我是{WhoAreYou},请输入你的问题 (q退出, help帮助, 回车重复上一个问题): ")
        query = input(": ").strip()
        ...
        # 记录历史
        # 这里只能简单记录，若rag_chain返回答案可直接获取
        # 假设test_rag最后一次输出为AI答案
        # 这里可进一步优化为返回答案字符串
